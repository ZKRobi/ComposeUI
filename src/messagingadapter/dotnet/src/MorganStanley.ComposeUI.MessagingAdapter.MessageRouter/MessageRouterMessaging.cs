...
using MessageHandler = MorganStanley.ComposeUI.Messaging.Abstractions.MessageHandler;
...
internal class MessageRouterMessaging : IMessaging
{
    ...
    /// <param name="cancellationToken">A cancellation token to observe while waiting for the task to complete.</param>
    /// <returns>A <see cref="ValueTask"/> representing the asynchronous operation.</returns>
    public async ValueTask<IAsyncDisposable> RegisterServiceAsync(string serviceName, ServiceHandler subscriber, CancellationToken cancellationToken = default)
    {
